...
if(isset($_GET['e_attend_ID'])){
    $id = $_GET['e_attend_ID'];
    $sql = "DELETE FROM `t_attend` WHERE `t_attend`.`e_attend_ID` = '$id'";
    $sql1 = "DELETE FROM `t_attend_pa` WHERE `t_attend_pa`.`e_attend_id` = '$id'";
    $result = $conn->query($sql);
    $result1 = $conn->query($sql1);

     if ($result or $result1 ) { 
         echo "<script>alert('Record deleted successfully');location='../view_trainer_attendance.php'</script>";
     } else {

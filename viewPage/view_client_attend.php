...
if(isset($_GET['submit'])){
   $date = $_GET['date'];
    # Create a connection
    $ch = curl_init();
    curl_setopt( $ch, CURLOPT_URL, "http://localhost/yoga_project/Viewapi/view_client_attend_api.php/?date=$date");
    curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true);
    # Get the response
    $content = curl_exec($ch);
    $client_attend = json_decode($content);
    $client_attend_view = $client_attend->client_attend;
    print_r($client_attend_view);
    }
?>
<div class="content">
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="content">
                <form action="view_client_attend.php" method="GET">
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="panel panel-default templatemo-content-widget white-bg no-padding templatemo-overflow-hidden" style="overflow:auto;">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered"  >
                                ...
                                <tbody id="myTable"><?php $i=0;foreach($client_view as $value): ?><?php foreach($client_attend_view as $value1):?>
                                    <?php $cid=$value1->c_id; $c_ID=explode(',',$cid);?>
                                    <?php $array = (array) $value;
                                    if(in_array ($c_ID[$i],$array)){  ?>
                                ...
                                <input type="hidden" value="<?php echo $date;?>" name="date" >
                                    <tr>
                                       <td><?php $i++;echo $i; ?></td>
                                        ...
                                        <td><?php if($value1->attendance==1){echo "present";}else{echo "Absent";}?>
                                            
                                        </td>
                                        <td><?php echo $value->package; ?></td>
                                        <td><?php echo $value1->in_time ;?></td> 
                                        <td><?php echo $value1->out_time ; ?></td>
                                    </tr><?php }?><?php endforeach; ?><?php endforeach;?>
                                </tbody>
                            </table>
                        </div>
                    <center><button type="submit" name="submit" class="btn btn-success"style="margin-top: 8px;">Submit</button></center>

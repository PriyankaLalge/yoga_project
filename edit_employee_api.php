...
if(isset($_POST['e_ID'])/* && isset($_POST['e_name']) && isset($_POST['e_surname']) && isset($_POST['address']) && isset($_POST['contact'])*/ ){
    ...
    $sql = "UPDATE `employee` SET `e_name` = '$e_name', `e_surname` = '$e_surname', `Gender` = '$Gender', `DOB` = '$DOB', `Age` = '$Age', `Title` = '$Title', `Salary` = '$Salary', `Register_ID` = '$Register_ID', `address` = '$address', `contact` = '$contact', `Email` = '$Email', `status` = 'unpaid' WHERE `employee`.`e_ID` = $eid";

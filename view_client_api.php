...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        array_push($client,array('c_ID'=>$row['c_ID'],'c_name'=>$row['c_name'],'c_surname'=>$row['c_surname'],'gender'=>$row['gender'],'DOB'=>$row['DOB'],'Anniversary'=>$row['Anniversary'],'Age'=>$row['Age'],'address'=>$row['address'],'contact'=>$row['contact'],'fees'=>$row['fees'],'received'=>$row['received'],'balance'=>$row['balance'],'package'=>$row['package'],'Lead_By'=>$row['Lead_By'],'photo'=>$row['photo'],'Comments'=>$row['Comments'],'status_payment'=>$row['status_payment'],'batch_id'=>$row['batch_id'],'startdate'=>$row['startdate'],'enddate'=>$row['enddate']));
    }
